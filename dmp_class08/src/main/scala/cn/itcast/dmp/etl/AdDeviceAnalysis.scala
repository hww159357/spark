...
object AdDeviceAnalysis extends ProcessData{
  ...
  private val destTable="adDeviceAnalysis"
  ...
  override def process(sparkSession: SparkSession, kuduContext: KuduContext): Unit = {
    ...
    //2、数据处理分析
    val adDeviceAnalysis: DataFrame = sparkSession.sql(ContantsSQL.deviceAnalysis_temp)

    adDeviceAnalysis.createTempView("adDeviceAnalysis")
    val finalResult: DataFrame = sparkSession.sql(ContantsSQL.deviceAnalysis)

    //3、把结果数据保存到kudu中
    val schema: Schema = ContantsSchema.deviceAnalysisSchema
    //分区id
