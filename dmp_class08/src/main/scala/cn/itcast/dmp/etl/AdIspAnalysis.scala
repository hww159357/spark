...
object AdIspAnalysis extends ProcessData{
  ...
  private val destTable="adIspAnalysis"
  ...
  override def process(sparkSession: SparkSession, kuduContext: KuduContext): Unit = {
    ...
    //2、数据处理分析
    val ispAnalysis: DataFrame = sparkSession.sql(ContantsSQL.ispAnalysis_temp)

    ispAnalysis.createTempView("ispAnalysis")
    val finalResult: DataFrame = sparkSession.sql(ContantsSQL.ispAnalysis)

    //3、把结果数据保存到kudu中
    val schema: Schema = ContantsSchema.ispAnalysisSchema
    //分区id
